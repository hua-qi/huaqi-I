...
> **注意（2026-04-09 更新）**：定时任务系统已完整重构，本文档中"定时任务配置"（`SchedulerJobConfig`、`register_default_jobs`、`_DEFAULT_JOB_CONFIGS`）和"scheduler CLI 命令"（`enable`/`disable`/`set-cron`）相关内容已过时。当前实现请参阅：
