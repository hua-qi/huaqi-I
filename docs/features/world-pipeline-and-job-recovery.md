...
> **注意（2026-09-04 更新）**：定时任务系统已完整重构，本文档中"定时任务配置"（`SchedulerJobConfig`、`register_default_jobs`、`_DEFAULT_JOB_CONFIGS`）和"scheduler CLI 命令"（`enable`/`disable`/`set-cron`）相关内容已过时。当前实现请参阅：
> - [`docs/features/scheduler.md`](./scheduler.md) — 定时任务系统完整文档
> - [`docs/design/adr/ADR-007-scheduler-unified-chat-tasks.md`](../design/adr/ADR-007-scheduler-unified-chat-tasks.md) — 架构决策记录
>
> `WorldPipeline`、`JobExecutionLog`、`MissedJobScanner`、`StartupJobRecovery` 等模块本身仍有效，文档描述准确。
