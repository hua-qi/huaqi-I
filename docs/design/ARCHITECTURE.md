...
│   ├── tools.py                  # Agent 可调用工具注册
│   └── hooks.py                  # 对话后 Hook 触发
│
├── cli/                          # CLI 层（Typer）
│   ├── __init__.py               # app 挂载点，组装所有子命令
│   ├── ui.py                     # 命令补全、输入框等 UI 组件
│   ├── ui_utils.py               # Rich 终端 UI 工具集
│   ├── notifier.py               # CLI 通知器
│   ├── context.py                # 全局组件缓存与 ensure_initialized
│   ├── chat.py                   # 对话主循环与斜杠命令处理
│   └── commands/                 # 子命令（每命令一文件）
│       ├── config.py / profile.py / pipeline.py
│       ├── personality.py / people.py / study.py
│       └── collector.py / proxy.py / system.py / onboarding.py
│
├── config/                       # 配置管理（统一入口）
│   ├── manager.py                # 配置加载/保存，get_config_manager()
│   ├── paths.py                  # 数据目录路径函数
│   ├── hot_reload.py             # watchdog 热重载
│   ├── users.py                  # 用户管理
│   ├── errors.py                 # 配置相关异常
│   └── adapters/                 # 适配器抽象基类
│       ├── interface_base.py / scheduler_base.py
│       ├── storage_base.py / storage.py / vector_base.py
│
├── layers/                       # 三层架构（核心业务）
│   │
│   ├── data/                     # 数据层：收、存、不丢
│   │   ├── collectors/           # 外部数据采集（CLI 对话、微信）
│   │   ├── diary/                # 日记存储（store.py）
│   │   ├── events/               # 事件模型与 SQLite 存储
│   │   │   ├── models.py         # Event dataclass + redact_sensitive_info
│   │   │   └── store.py          # LocalDBStorage
│   │   ├── flexible/             # 灵活键值存储（JSONL + Chroma）
│   │   │   └── store.py          # FlexibleStore, AnalysisResult
│   │   ├── git/                  # Git 自动提交
│   │   │   └── auto_commit.py    # GitAutoCommit
│   │   ├── memory/               # 记忆检索
│   │   │   ├── search/           # BM25 + LLM 混合检索
│   │   │   ├── storage/          # Markdown 持久化存储
│   │   │   └── vector/           # Chroma 向量数据库封装
│   │   ├── profile/              # 用户画像（数据层部分）
│   │   │   ├── models.py         # UserIdentity/Preferences/Background/Profile
...
│   └── growth/                   # 成长层：理解、提炼、更新
│       └── telos/                # TELOS 知识图谱
│           ├── models.py / manager.py / engine.py
│           ├── growth_events.py / meta.py / context.py
...
### 3.7 记忆系统 (`huaqi_src/layers/data/memory/`)
...
### 3.3 人格引擎 (`huaqi_src/layers/capabilities/personality/`)
...
### 3.4 成长系统 (`huaqi_src/layers/capabilities/growth/`)
...
### 3.5 内容流水线 (`huaqi_src/layers/capabilities/pipeline/`)
...
| 文件路径 | 作用 | 重要程度 |
|----------|------|----------|
| `cli.py` | CLI 入口（18行薄包装，挂载 huaqi_src/cli/app） | ⭐⭐⭐⭐⭐ |
| `huaqi_src/cli/__init__.py` | 子命令组装，主 app 定义 | ⭐⭐⭐⭐⭐ |
| `huaqi_src/cli/chat.py` | 对话主循环、斜杠命令处理 | ⭐⭐⭐⭐⭐ |
| `huaqi_src/cli/context.py` | 全局组件缓存、ensure_initialized | ⭐⭐⭐⭐ |
| `huaqi_src/agent/graph/chat.py` | 对话工作流状态图 | ⭐⭐⭐⭐⭐ |
| `huaqi_src/agent/state.py` | 统一 Agent 状态定义 | ⭐⭐⭐⭐⭐ |
| `huaqi_src/agent/nodes/chat_nodes.py` | 对话节点实现 | ⭐⭐⭐⭐ |
| `huaqi_src/config/manager.py` | 配置加载/保存，get_config_manager() | ⭐⭐⭐⭐ |
| `huaqi_src/config/paths.py` | 所有数据目录路径函数（统一入口） | ⭐⭐⭐⭐⭐ |
| `huaqi_src/layers/data/profile/models.py` | 用户画像数据模型 | ⭐⭐⭐⭐ |
| `huaqi_src/layers/data/profile/manager.py` | 画像持久化与更新 | ⭐⭐⭐⭐ |
| `huaqi_src/layers/data/profile/narrative.py` | LLM 叙事性画像生成 | ⭐⭐⭐⭐ |
| `huaqi_src/layers/capabilities/personality/engine.py` | 五维人格引擎 | ⭐⭐⭐⭐ |
| `huaqi_src/layers/capabilities/llm/manager.py` | LLM 抽象（多提供商） | ⭐⭐⭐⭐⭐ |
| `huaqi_src/layers/data/memory/search/` | BM25 + LLM 混合检索 | ⭐⭐⭐⭐⭐ |
| `huaqi_src/layers/capabilities/pipeline/core.py` | 内容流水线主逻辑 | ⭐⭐⭐⭐ |
...
| `docs/guides/code-standards.md` | 代码及文件组织规范（agent 参考） | ⭐⭐⭐⭐⭐ |
...
**文档版本**: v1.3
**生成时间**: 2026-02-04
**更新说明**: 删除 core/ 万能桶，全部迁入 layers/ 三层架构；config/ 统一配置管理；更新源码目录树与关键文件索引
