...
| `/skills` | 查看技能列表 |
| `/goals` | 查看目标列表 |
| `/report [类型] [日期]` | 查看或生成报告 (morning/daily/weekly/quarterly/insights) |
| `/care` | 手动触发关怀检查 |
| `/care status` | 查看关怀统计 |
| `/history` | 最近对话记录（传统模式）|
...
统一管理晨间简报、日终复盘和周报等报告的查看与生成。

```bash
huaqi report morning               # 查看或生成今天的晨间简报
huaqi report daily                 # 查看或生成今天的日终复盘
huaqi report weekly                # 查看或生成本周的周报

# 指定日期查看历史报告（支持 yesterday 或 YYYY-MM-DD）
huaqi report morning yesterday
huaqi report daily 2024-03-01
...
**数据存储路径：**
报告文件通常存储在 `{data_dir}/reports/` 目录下。
