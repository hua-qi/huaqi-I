...
| 改动类型 | 必须产出 |
|----------|---------|
| 新功能 | `docs/specs/` → `docs/plans/` → tests → code → `docs/features/` |
| 功能增强（影响已有行为） | `docs/plans/` → tests → code → 更新 `docs/features/` |
| Bug fix | tests（复现）→ code → `CHANGELOG.md` |
| 重构（不改变行为） | tests（保护）→ code |
| 架构变更 | `docs/design/adr/ADR-xxx.md` → `docs/design/ARCHITECTURE.md` |
